...
// Fundo animado de linhas (react-bits FloatingLines, port para TS).
// Renderiza um shader WebGL via three. Decorativo.
...
export default function FloatingLines({
  ...
  const getLineCount = (waveType: string): number => {
    ...
    if (!enabledWaves.includes(waveType as "top")) return 0;
    const index = enabledWaves.indexOf(waveType as "top");
    return lineCount[index] ?? 6;
  };
  const getLineDistance = (waveType: string): number => {
    if (typeof lineDistance === "number") return lineDistance;
    if (!enabledWaves.includes(waveType as "top")) return 0.1;
    const index = enabledWaves.indexOf(waveType as "top");
